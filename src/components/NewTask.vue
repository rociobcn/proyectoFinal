<template>
  <div class="w-full px-10 sm:px-20 lg:px-32 mb-14">
    <div class="flex sm:flex-col lg:flex-row justify-between mt-16">
      <div class="flex flex-col flex-1 w-full lg:w-60 mr-6">
        <h3 class="text-sky-600 font-bold text-2xl mb-4 w-1/2 mt-16">
          Welcome <span>{{ changeUserName() }}</span>
        </h3>
        <form @submit.prevent="addTodo" class="flex">
          ...
          <div class="flex flex-col sm:flex-row">
            <button
              class="bg-sky-600 hover:bg-sky-900 text-white font-bold py-2 px-4 rounded-full w-full ml-4 mb-2 sm:mb-0"
            >
              Add
            </button>
            <button
              class="bg-sky-600 hover:bg-sky-900 text-white font-bold py-2 px-4 rounded-full w-full ml-4 sm:ml-2"
              @click="clean"
            >
              Clean
            </button>
        ...
        <p v-if="newTodo.length < 4" class="mt-6 text-slate-600 font-bold">
          {{ newTodoErr }}
        </p>
      </div>
      <div class="hidden sm:flex justify-center mt-10 lg:mt-0">
      ...
      >
        <div class="">
          <input
            ...
            type="checkbox"
            v-on:change="completedTask(todo)"
        ...
        <div class="">
          <button
            @click="editTodo(i)"
            class="hover:bg-sky-600 text-white font-bold rounded-full w-12 mr-2"
          >
            📝
          </button>
          <button
            @click="removeTodo(todo)"
            class="hover:bg-sky-600 text-white font-bold rounded-full w-12"
          >
            🗑️
          </button>
        </div>
    ...
    <div class="w-full bg-gray-200 rounded-full mt-6 flex">
      <div
        class="bg-sky-600 text-xl font-medium text-white font-bold text-center p-2 leading-none rounded-full"
    ...
    <button
      class="bg-sky-600 hover:bg-sky-900 text-white font-bold py-2 px-4 rounded-full lg:w-1/6 mt-6"
      @click="isCompleted = !isCompleted"
    >
      {{ isCompleted ? "Show all" : "Hide completed" }}
    </button>
    <h5 class="mt-6 text-slate-600 font-bold" v-if="datosTask.length === 0">
...
const showTasks = computed(() => {
  return isCompleted.value === true
    ? datosTask.value
    : datosTask.value.filter((t) => t.is_complete === false);
});
</script>
